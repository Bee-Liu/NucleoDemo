...
typedef enum
{
	CPU_ENTER_CRITICAL,		//CPU进入临界  关闭所有中断  保存系统当前运行状态
	CPU_EXIT_CRITICAL,		//CPU退出临界  打开所有中断  恢复系统之前运行状态
}CPU_EA_TYPEDEF;

//定义一个CPU中断控制回调函数指针,别名CPUInterrupt_CallBack_t,
typedef void (*CPUInterrupt_CallBack_t)(CPU_EA_TYPEDEF cmd,unsigned char *pSta);

//extern CPUInterrupt_CallBack_t CPUInterrupptCtrlCBS; //重新定义一个函数指针名
//系统任务ID
typedef enum
{
	OS_TASK1,
